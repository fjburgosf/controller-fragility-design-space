...
for k, (algo, c) in enumerate((("sac", AZUL), ("ddpg", NARANJA))):
    ...
    a.annotate(f"spread {peor/mejor:.1f}x", (len(s) - 1.4, peor * 0.72), fontsize=7.5,
               color=c, weight="bold")
...
a.legend(fontsize=8); a.grid(axis="y", alpha=0.3); a.tick_params(labelsize=8)
...
for algo, (c, best, final) in datos.items():
    ...
    a.text(pos + 0.16, max(best) + 6, f"{algo.upper()}\nn={len(best)}", ha="center", fontsize=7.5)
...
a.text(4.8, 0.62, "inside the training\ndisturbance range", ha="center", fontsize=7,
       color=VERDE, weight="bold")
a.text(8.3, 0.62, "outside", ha="center", fontsize=7.5, color=ROJO, weight="bold")
...
a.legend(fontsize=7.5, loc="upper left"); a.grid(alpha=0.3); a.tick_params(labelsize=8)

fig.suptitle("Fragility of learned control lives in its configuration and in its training distribution",
             fontsize=10.5, y=1.02)
fig.tight_layout()
for ext in ("png", "pdf"):
    fig.savefig(FIG / f"fig3_rl_fragility.{ext}", bbox_inches="tight", dpi=600)
print("fig3_rl_fragility guardada")
